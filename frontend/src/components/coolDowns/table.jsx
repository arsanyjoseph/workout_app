...
function GenerateTR (data) {
    ...
    if(users.length > 0) {
            ...
            <tr className='tableData' key={index}>
                ...
                <td>{idName}</td>
                <td>{creationDate}</td>
                <td className='assignedUsers'>{item.assignedUsersId.length > 0 ? item.assignedUsersId.map((i)=> <span key={index + i.userId}>{searchArray(i.userId, users) + ', '}</span> ) : 'No Users Assigned Yet'}</td>
            </tr>
                )  
            })}
            </>
